...
void get_tasks(task *t1, int n) //A função cria e pega os dados das tasks
{
	int i = 0;
	while (i < n)
	{
		printf("Enter Task %d parameters\n", i + 1);
		printf("Arrival time: ");//Não preciso
...
int hyperperiod_calc(task *t1, int n) //Não preciso dela
{
	int i = 0, ht, a[10];
	while (i < n)
...
int gcd(int a, int b) //Tem a ver com o hyperperiod Calc
{
	if (b == 0)
		return a;
	else
...
int lcm(int *a, int n) //Tem a ver com o hyperperiod calc
{
	int res = 1, i;
	for (i = 0; i < n; i++)
	{
...
int sp_interrupt(task *t1, int tmr, int n) // Testes para ver a interrrupção no processamento
{
	int i = 0, n1 = 0, a = 0;
	task *t1_copy;
	t1_copy = t1;
