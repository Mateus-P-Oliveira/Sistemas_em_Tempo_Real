...
int hyperperiod_calc(task *t1,int n);			//Calculate hyperiod period of the task set  //Não uso
float cpu_util(task *t1,int n);				//Calculates CPU utilization  
int gcd(int a, int b);					//Find greatest common divisor   //Não uso
int lcm(int *a, int n);					//Find Least common multiple   //Não uso
...
void update_abs_arrival(task *t1,int n,int k,int all);	//Update absolute arrival time (absolute arrival time = arrivaltime + instance*period) //Não uso
